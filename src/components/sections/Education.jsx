...
const EducationItem = ({ degree, institution, year, score, isLeft }) => (
    <div className={`relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group w-full ${isLeft ? '' : ''}`}>
        ...
        <div className="w-full md:w-[calc(50%-30px)] pl-12 md:pl-0">
            <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border-l-4 border-transparent bg-clip-padding relative shadow-sm hover:shadow-lg transition-all duration-300 group-hover:-translate-y-1">
                {/* Gradient Border Trick */}
                <div className="absolute inset-y-0 left-0 w-1 bg-gradient-to-b from-blue-600 to-purple-600 rounded-l-xl"></div>

                <div className="flex justify-between items-start mb-2">
                    <h3 className="text-xl font-bold text-slate-900 dark:text-white uppercase tracking-tight group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{degree}</h3>
                    {year && (
                ...
                {institution && (
                    <div className="flex items-center text-slate-700 dark:text-slate-300 font-medium mb-3">
                        <GraduationCap size={16} className="mr-2 text-purple-600 dark:text-purple-400" />
                        {institution}
                    </div>
                )}

                <div className="flex items-center text-slate-600 dark:text-slate-300 text-sm font-semibold bg-slate-50 dark:bg-slate-700 p-2 rounded-lg inline-block border border-slate-100 dark:border-slate-600">
                    <Award size={16} className="mr-2 text-pink-500" />
                    Score: <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-700 to-purple-700 dark:from-blue-400 dark:to-purple-400 font-bold ml-1">{score}</span>
                </div>
            </div>
        </div>
    </div>
        ...
        <section id="education" className="py-20 bg-white dark:bg-slate-900 transition-colors duration-300">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="text-center mb-16">
                    ...
                    <h2 className="text-3xl md:text-4xl font-bold text-slate-900 dark:text-white mb-4 uppercase tracking-tight hover:text-transparent hover:bg-clip-text hover:bg-gradient-to-r hover:from-blue-600 hover:to-purple-600 transition-all duration-300 cursor-default">Education</h2>
                ...
                <div className="relative max-w-4xl mx-auto space-y-8 md:space-y-12">
                    {/* Vertical Line */}
                    <div className="absolute left-4 md:left-1/2 top-0 bottom-0 w-0.5 bg-gradient-to-b from-blue-200 via-purple-200 to-pink-200 dark:from-blue-900 dark:via-purple-900 dark:to-pink-900 transform -translate-x-1/2"></div>
